...
def smith_hook(
    ...
    def smith_wrapper(*args, **kwargs):
        ...
        if filters and not any([f(*args, **kwargs) for f in filters]):
            return func(*args, **kwargs)

        if request_processor:
            _context.request = request_processor(*args, **kwargs)

        if inherit:
            locals().update(inspect.currentframe().f_back.f_locals)

        def stringify(obj):
            if processors and type(obj) in processors:
                return processors[type(obj)](obj)
        ...
        smith_trace = {
            ...
            'request': json.dumps(getattr(_context, 'request', {})),
            'stack_trace': [
        ...
        if can_block:
            ...
            if _block and any(pred(rule) for rule in _block['rules']) and not any(
                    any(i in j for j in smith_trace['stack_trace']) for i in _block.get('unstoppable', [])):
                smith_trace['blocked'] = True
                _client.post_message(TRACE_OPERATE, smith_trace)
                raise RuntimeError('API blocked by RASP,for any questions:  https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=288h9a9a-8791-4bb1-a7b1-45f54489ac15')
